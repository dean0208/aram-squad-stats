...
export default function DailyReceipt({
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      text('마 좀 치나?', 64, 100, 38, '#2563eb', 800)
      text('우리의 나락 영수증', 64, 150, 22)
      text(`${displayDate(date)} · KST`, 64, 188, 18, '#526176')
      rule(220)
      text(
      ...
      text(line, 64, 396, 24, '#2563eb', 700)
      rule(428)
      text('이날 최고의 한 판', 64, 472, 17, '#526176')
      text(best, 64, 508, 26, '#875414', 700)
      text(`평소보다 잘한 사람  ${rise}`, 64, 555, 20, '#16734d')
      text(`이날의 걸배이  ${anchor}`, 64, 595, 20, '#9f3c50')
      rule(629)
      text('함께한 사람들', 64, 669, 20, '#182333', 700)
      ;[...counts.values()].forEach((player, index) => {
        const y = 716 + index * 58
        text(player.name, 64, y, 22, '#182333', 700)
        text(
      ...
      rule(944)
      text('기록은 남고, 다음 판은 온다.', 64, 984, 19, '#526176')
      text(`${window.location.host}/?date=${date}`, 64, 1024, 17, '#2563eb')
