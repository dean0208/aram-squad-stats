...
- 홈은 날짜별 전적 → 시상식 → 경기 → 개인 프로필 → 우리 기록 순서다. 날짜와 펼친 경기는 `?date=YYYY-MM-DD&game=...`에 보존한다.
- **전적 영수증**에서 별명·승패·시상·메달을 담은 PNG를 미리 보고 저장하거나 해당 날짜 링크를 복사한다. 이미지는 브라우저에서 생성한다.
- **시상식 다시 보기**는 직접 닫을 때까지 유지한다. 새 수상자의 자동 연출은 약 2초이며 브라우저별로 켜고 끌 수 있다.
- **우리 기록**은 선택 날짜까지의 주간(월요일 시작)·월간·조회한 전체 기록을 비교한다. 공동 1위와 도전자 격차를 표시한다.
- **개인 기록 경신·반등**은 이전 경기와 비교하고 근거 경기에 연결한다. 반등은 직전 50점 미만에서 15점 이상 상승한 경우다.
- **챔피언 도감**은 검색·정렬·최소 경기 수·점수 구간·미도전 필터와 최고점·첫 승 링크를 제공한다. 최근 폼 막대를 누르면 경기 정보를 확인할 수 있다.
- 경기 상세는 모바일에서 선수별 카드로 표시하며, 증강 한국어 이름과 메달 수상 기준을 제공한다. 모든 경기 시간은 한국 시간으로 표시한다.

현재 화면의 기록 범위는 공통 조회 레이어의 최근 최대 500경기다. 기여도 산식과 경기 수집 방식은 그대로 사용한다.
...
아이템 데이터는 수집·전송·저장하지 않으며 점수에도 사용하지 않는다.
`item_ids` 컬럼 없이 경기 저장이 가능하다. 과거 아이템 컬럼 마이그레이션은
이력으로만 남아 있으며, 현재 동기화를 위해 적용할 필요가 없다.
