...
남은 길은 둘이다.
1. **LCU 에이전트** — 로컬 클라이언트가 들고 있는 만큼은 백필할 수 있다.
   `python lcu_agent.py --debug` 의 `[5]` 절이 이 클라이언트가 어떤 지표를
   알려주는지, 경기가 언제까지 남아 있는지 출력한다.
2. **프로덕션 API 키** — Riot 에 신청해 큐 2400 접근을 받으면 전부 채울 수 있다.
