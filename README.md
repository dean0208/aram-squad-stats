...
`shields_on_teammates` · `hard_cc_count` · `damage_self_mitigated` · `cc_score` 는
전부 Riot Match-V5 응답에 원래 있는 필드라, 처음에는 `match_id` 로 다시 읽어
과거 221경기를 채울 수 있을 줄 알았다. `/api/backfill` 이 그 목적으로 있다.

**되지 않는다.** 진단해 보니 이 API 키는 **큐 2400(ARAM Mayhem) 경기에 접근
자체가 안 된다**:

```
ids(큐 필터 없음)  → 200, 최신이 2026-07-12 (큐 480 경기)
ids(queue=2400)   → 200, 0건
매치 상세 (큐 480)  → 200
매치 상세 (큐 2400) → 403 Forbidden   ← 저장된 221경기가 전부 여기
```

라우팅 문제가 아니다 (`sea` 가 맞고 나머지 셋은 404). 레이트리밋도 아니다
(100:120 중 2건 사용). 키도 살아 있다. **큐 2400 만 안 준다.**

그래서 2026-07-12 이후 경기는 Riot 경로로 아예 들어오지 않았고, 지금 쌓이는
경기는 전부 LCU 에이전트가 넣은 것이다. 매시간 워크플로가 계속 success 로
찍힌 건 `synced: 0` 도 성공으로 보기 때문이다.

### LCU 로도 안 된다 (실측)

리그 클라이언트를 켠 채 `python lcu_agent.py --debug` 로 확인했다. LCU 의
match-history 는 **구 match-v4 포맷**이라 `challenges` 블록 자체가 없다.

| 지표 | LCU 제공 | 비고 |
|---|---|---|
| 막아낸 피해 | **O** | `damageSelfMitigated` |
| CC 지속시간 | **O** | `totalTimeCrowdControlDealt` |
| 팀원 힐 | X | `totalHeal`(자힐 포함)만 있다 |
| **팀원 실드** | **X** | 어떤 키로도 없다 |
| 하드CC 횟수 | X | `challenges` 블록이 없다 |

게다가 클라이언트가 들고 있는 Mayhem 경기는 **9건, 2026-09-05 까지**다.
백필할 과거가 애초에 없다.

### 정리

- **막아낸 피해**는 지금부터 LCU 로 들어온다 — 탱킹 축은 살아난다.
- **실드와 하드CC 는 어느 경로로도 못 받는다.** 코드는 값이 오면 바로 쓰도록
  되어 있고, 없는 동안에는 남은 신호만으로 돌아 점수가 흔들리지 않는다.
- 이 셋을 다 열고 과거 221경기까지 채우는 방법은 **프로덕션 API 키 하나뿐**이다
  (Riot 에 신청해 큐 2400 접근을 받으면 Match-V5 의 모든 필드가 함께 열린다).
  `/api/backfill` 은 그때 코드 변경 없이 그대로 돌아가도록 남겨 두었다.

### 재보정 시점은 스크립트가 알려준다

새 기준 경기가 얼마나 쌓였는지, 계수를 다시 맞출 때가 됐는지 기억에 맡기지
않는다.
